	...
	<g>
		...
		>
			<div
				...
				class="@container flex size-full flex-nowrap justify-center gap-1 px-2 py-0.5 text-sm"
			>
				<!-- https://stackoverflow.com/a/42551367/4617687 -->
				<p
					dir="rtl"
					class="shrink truncate text-gray-900"
				>
					&lrm;{{ content.name }}
				</p>
				<p class="hidden shrink-0 text-gray-500 @2xs:block">—</p>
				<p class="hidden shrink-0 text-gray-500 @2xs:block">{{ formattedSize }}</p>
			</div>
		</foreignObject>
...
const percentageOfTotal = computed(() =>
	Math.min((props.content[props.compressionType] / props.totalBytes) * 100, 100)
);
const hoverData = computed(
	() => `${props.content.name} — ${formattedSize.value} (${percentageOfTotal.value.toFixed(2)}%)`
);
...
const children = computed<Array<Content>>(() => {
	if (!isFolder(props.content) || height.value <= tresholdInPixels || width.value <= tresholdInPixels) {
		return [];
	}
