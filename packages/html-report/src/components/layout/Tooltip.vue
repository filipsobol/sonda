<template>
	<div
		...
		:style="{ '--x': x, '--y': y }"
		role="tooltip"
		class="pointer-events-none fixed z-10 max-w-[400px] rounded-md bg-gray-800 px-2 py-1 break-words text-gray-100"
	>
		<span>{{ content }}</span>
	</div>
</template>
...
function onMouseMove(event: MouseEvent) {
	...
	const calcX = clientX + width.value + margin > bodyWidth.value ? clientX - width.value - margin : clientX + margin;
	const calcY = clientY + height.value + margin > bodyHeight.value ? clientY - height.value : clientY + margin;
