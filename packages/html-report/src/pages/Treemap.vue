	...
	<div class="flex h-full flex-col">
		<div class="max-w-7xl">
			<template v-if="activeAsset">
				...
				<p class="mt-4 text-gray-500">
					Treemap representation of the
					<span class="font-semibold">{{ activeAsset.name }}</span> asset.
				</p>
			</template>
			...
			<template v-else>
				...
				<p class="mt-4 text-gray-500">Treemap representation of the build, showing all assets and their sizes.</p>
			</template>
		</div>
		...
		>
			...
			>
				...
				<template #body>
					<p>There are no inputs matching the current filters.</p>
				</template>
			</Alert>
		</div>
	</div>
...
const content = computed(() => {
	...
	resources.value
		.filter(
			resource => !normalizedSearch || isNegativeSearch !== resource.name.toLocaleLowerCase().includes(normalizedSearch)
		)
