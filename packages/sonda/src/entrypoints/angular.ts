...
export default async function SondaAngular({
	...
	for (const project of projectsToGenerate) {
		...
		const paths: Paths = typeof outputPath === 'object' ? outputPath : { base: outputPath };
		...
		const metafile = updateMetafile(loadJson<Metafile>(resolve(paths.base, 'stats.json')), paths.base);
 ...
 */
function updateMetafile(metafile: Metafile, basePath: string): Metafile {
