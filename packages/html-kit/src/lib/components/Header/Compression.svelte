...
<script lang="ts">
import { store, type CompressionType } from '$lib/store.svelte.js';

const hasGzip = $derived( store.output?.root.gzip ?? 0 > 0 );
const hasBrotli = $derived( store.output?.root.brotli ?? 0 > 0 );
